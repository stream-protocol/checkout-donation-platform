...
function ConfigCard(props) {
    ...
    switch (config.type) {
        case "single":
            ...
            switch (option.type) {
                case "roundup":
                    editorComponent = < RoundUpEditor config = { config }
                    setConfig = { setConfig }
                    />;
                    break;
                case "fixed":
                    editorComponent = < FixedEditor config = { config }
                    setConfig = { setConfig }
                    />;
                    break;
                case "input":
                    editorComponent = < InputEditor config = { config }
                    setConfig = { setConfig }
                    />;
                    break;
                default:
            ...
            break;
        case "multi_type":
            editorComponent = < MultiTypeEditor config = { config }
            setConfig = { setConfig }
            />;
            break;
        case "multi_recipient":
            editorComponent = < MultiRecipientEditor config = { config }
            setConfig = { setConfig }
            />;
            break;
        case "custom":
            editorComponent = < CustomEditor config = { config }
            setConfig = { setConfig }
            />;
            break;
        default:
    ...
    return ( <
            div >
            <
            Card variant = { isActive ? "outlined" : "elevation" }
            sx = { isActive ? { borderColor: "success.light", borderWidth: 2 } : {} } >
            <
            CardContent >
            <
            ...
            color = { editing ? "success" : "primary" } > { editing ? "Save" : "Edit" } <
            /Button> <
            Button disabled = { editing }
            onClick = { handleActiveToggleDialogClickOpen } > { isActive ? "Set as inactive" : "Set as active" } <
            /Button> <
            Dialog open = { activeToggleDialogOpen }
            onClose = { handleActiveToggleDialogClose }
            maxWidth = "xs" >
            <
            DialogTitle > {
                isActive ?
                'Set "' + config.name + '" as inactive?' : 'Set "' + config.name + '" as your active configuration?'
            } <
            /DialogTitle> <
            DialogContent >
            <
            DialogContentText > {
                isActive ?
                "Your checkout systems will stop using this donation configuration immediately." : "Your checkout systems will begin using this donation configuration immediately."
            } <
            /DialogContentText> < /
            DialogContent > <
            DialogActions >
            ...
            Button onClick = { handleActiveToggleConfirm }
            autoFocus >
            ...
            /Dialog> <
            Box sx = {
                { display: 'flex', flexGrow: 1, justifyContent: "flex-end", flexDirection: "row" }
            } >
            <
            Button disabled = { editing }
            color = "error"
            ...
            <
            Box sx = {
                { mt: 1, display: 'flex', flexGrow: 1, justifyContent: "center", flexDirection: "row" }
            } >
            <
            CheckCircleOutlineIcon sx = {
                { color: 'success.light', mr: 1 }
            }
            /> <
            ...
            color = "success.light" > ACTIVE < /Typography> < /
            Box >
        } <
        /div>
);
