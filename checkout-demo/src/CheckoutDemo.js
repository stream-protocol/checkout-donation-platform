...
function CheckoutDemo() {
    ...
    return ( <
        Box sx = {
            { background: theme.palette.grey[100], height: '100vh' }
        } >
        <
        ...
        gutterBottom > Shipping Address < /Typography> <
        ...
        <
        Typography variant = "h5"
        gutterBottom > Billing Address < /Typography> <
        ...
        divider = { < Divider / > } > {
            items.map(
                (item, index) => < CheckoutItem item = { item }
                key = { index }
                />
            )
        } <
        ...
        Box >
        <
        DonationComponent merchantPublicKey = { publicKey }
        ...
        sx = {
            { pr: 1 }
        } > Buy with < /Typography><img src={logo} alt="Pay with Solana Pay" / >
        <
        /Button> < /
        Box > <
        /Paper> < /
